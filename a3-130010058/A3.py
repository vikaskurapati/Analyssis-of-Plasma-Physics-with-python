...
def ques1(tstep, tf, plot = 0, e_field = np.zeros(3)):
    ...
    euler_error = np.sum((euler_pos - exact_pos)**2)**0.5/radius
    print euler_error
    eu_en_err = euler_en[time] / 0.5*mass*10**8 -1
    print euler_error.shape

    euler_sem = euler_semi(charge, mass, e_field, b_field, v0, tstep, time)
    euler_pos2 = euler_sem[0]
    euler_en2 = euler_sem[1]
    euler_error2 = np.sum((euler_pos2 - exact_pos)**2, axis = 0)**0.5/radius
    eu_en_err2 = euler_en2[time] / 0.5*mass*10**8 - 1
    
    rk2 = rk_2(charge, mass, e_field, b_field, v0, tstep, time)
    rk2pos = rk2[0]
    rk2en = rk2[1]
    rk2_error = np.sum((rk2pos - exact_pos)**2, axis = 0)**0.5/radius
    rk2en_err = rk2en[time] / 0.5*mass*10**8 - 1
    
    boris = boris_push(charge, mass, e_field, b_field[2], 0, v0, tstep, time)
    bor_pos = boris[0]
    bor_en = boris[1]
    bor_error = np.sum((bor_pos - exact_pos)**2, axis = 0)**0.5/radius
    boren_err = bor_en[time] / 0.5*mass*10**8 - 1
    ...
    return [euler_error[time], eu_en_err, euler_error2[time], eu_en_err2, rk2_error[time], rk2en_err, bor_error[time], boren_err]
